...
export default {
  startsWith: "reject-model-",
  async execute(client: DiscordClient, interaction: Interaction) {
    try {
      if (
        !interaction.isButton() ||
        !interaction.customId.startsWith("reject-model-")
      )
        return;
      const submissionId = interaction.customId.slice("reject-model-".length);
      ...
      const messageModal = new ModalBuilder()
        .setCustomId(`rejection-message-${id}`)
      ...
      console.log("Error - reject model submission button interaction", error);
    }
  },
} as ButtonExecute;
